...
{
    [P2PMessage("Chat")] public record Chat([property: P2PProperty(1)] string User, [property: P2PProperty(2)] string Text);
    ...
    public async Task FanOut_Message_Reaches_All_Peers(SerializationProtocol proto)
    {
    ...
    busC.SubscribeTo<Chat>(c => { lock (receivedC) receivedC.Add(c); });
        ...
        {
            LocalPort = portA,
            OutboundEndPoints = [new IPEndPoint(IPAddress.Loopback, portB), new IPEndPoint(IPAddress.Loopback, portC)],
            SerializationProtocol = proto
        });
    ...
    {
        // Arrange star topology: central hub (H) listens; two spokes (S1,S2) send to the hub; the hub fans out to both.
        var basePort = GetEphemeralBasePort();
    ...
    s2.SubscribeTo<Chat>(c => { lock (s2Received) s2Received.Add(c); });
        ...
        {
            LocalPort = portHub,
            OutboundEndPoints = [new IPEndPoint(IPAddress.Loopback, portS1), new IPEndPoint(IPAddress.Loopback, portS2)],
            SerializationProtocol = proto
        });
        await s1.StartListeningAsync(new ProtocolConfiguration
        {
            LocalPort = portS1,
            OutboundEndPoints = [new IPEndPoint(IPAddress.Loopback, portHub)],
            SerializationProtocol = proto
        });
        ...
        // Assert each spoke received the hub reply.
    ...
    [P2PMessage("Person")] public record Person([property: P2PProperty(1)] string Name, [property: P2PProperty(2)] int Age, [property: P2PProperty(3)] Address Address);
    [P2PMessage("Address")] public record Address([property: P2PProperty(1)] string Street, [property: P2PProperty(2)] string City);

    [Theory]
    [InlineData(SerializationProtocol.None)]
    [InlineData(SerializationProtocol.JsonRaw)]
    [InlineData(SerializationProtocol.LengthPrefixed)]
    public async Task FanOut_With_Duplicate_Endpoints_Does_Not_Double_Send(SerializationProtocol proto)
    {
        var basePort = GetEphemeralBasePort();
        ...
        // Duplicate outbound endpoints must receive one frame each.
        await sender.StartListeningAsync(new ProtocolConfiguration
        {
            LocalPort = portSender,
            OutboundEndPoints = [new IPEndPoint(IPAddress.Loopback, portR1), new IPEndPoint(IPAddress.Loopback, portR2), new IPEndPoint(IPAddress.Loopback, portR2)],
            SerializationProtocol = proto
        });
    ...
    [InlineData(SerializationProtocol.JsonRaw)]
    [InlineData(SerializationProtocol.LengthPrefixed)]
    public async Task Concurrent_FanOuts_All_Messages_Delivered_Exactly_Once(SerializationProtocol proto)
    {
        // 4 peers each sending to the other 3.
        var basePort = GetEphemeralBasePort();
        int peerCount = 4;
        ...
        for (int i = 0; i < peerCount; i++)
        {
            ...
            var outboundEndpoints = others.Select(x => new IPEndPoint(IPAddress.Loopback, ports[x])).ToArray();
    ...
    [InlineData(SerializationProtocol.JsonRaw)]
    [InlineData(SerializationProtocol.LengthPrefixed)]
    public async Task Mixed_Message_Types_FanOut_Correctly(SerializationProtocol proto)
    {
        var basePort = GetEphemeralBasePort();
        ...
        await sender.StartListeningAsync(new ProtocolConfiguration
        {
            ...
            OutboundEndPoints = [new IPEndPoint(IPAddress.Loopback, recv1Port), new IPEndPoint(IPAddress.Loopback, recv2Port)],
            SerializationProtocol = proto
        });
